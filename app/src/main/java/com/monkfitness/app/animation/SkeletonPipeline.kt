 ...
 * full ordered stage pipeline (`pose.build()` → `ConstraintSolver.solve` → `finalizer.finalize()`),
 * exactly as described below. The rendered frame is unchanged from the pre-M2 baseline.
 *
...
class SkeletonPipeline(
     ...
     * only) → `finalizer.finalize(...)` → FK flatten. Returns a [PipelineResult] with a `null` report
     * (use [produceFrameValidated] for validation).
     */
