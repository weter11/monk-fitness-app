...
/**
 * DeepOverheadSquatPose — Biomechanics validation snapshot.
 *
 * Full-depth squat (heels planted, knees tracking over the feet, pelvis
 * below the knees), spine upright, thoracic extended, arms completely
 * overhead with elbows locked, wrists neutral, gaze forward.
 *
 * This pose intentionally stresses almost every joint at once, so any
 * remaining architectural defect (proportion, IK, rotation, symmetry)
 ...
 *  - bakeIkLimb with frame-relative pole vectors (squat legs + overhead arms)
 *  - corrected IK + corrected proportions across the whole body
 *
 * Joints deliberately stressed: ankles (mobility), knees (tracking),
 * pelvis (depth), lumbar (compensation check), thoracic (extension),
 * shoulders (overhead mobility), elbows (locked), wrists (neutral).
 */
class DeepOverheadSquatPose : BasePose() {
    ...
    override fun build(context: PoseContext): SkeletonPose {
        ...
        // Full-depth squat, centered, spine upright (no lean).
        val pelvisY = 60f
        val pelvisX = 0f
        ...
        // Legs: full-depth squat, heels planted, knees tracking outward over the feet.
        val ankleY = 25f
        targetF.set(0f, ankleY, -def.hipWidth * 1.5f)
        targetB.set(0f, ankleY, def.hipWidth * 1.5f)
        poleF.set(1f, 0f, -0.2f)
        poleB.set(1f, 0f, 0.2f)
        ...
        // Arms: completely overhead, elbows locked, wrists neutral.
        ...
        targetA.set(0f, shoulderY + armReach - 2f, -def.shoulderWidth * 0.5f)
        targetP.set(0f, shoulderY + armReach - 2f, def.shoulderWidth * 0.5f)
        poleA.set(0f, -1f, -1f)
        poleP.set(0f, -1f, 1f)
