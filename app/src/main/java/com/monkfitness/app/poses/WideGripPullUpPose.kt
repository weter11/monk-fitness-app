...
class WideGripPullUpPose : PoseBuilder {
    override val metadata = PoseMetadata(
        ...
        environment = EnvironmentDefinition(
            ...
            // Extra wide bar to accommodate the grip
            props = listOf(BoxProp(center = Vector3(0f, 500f, 15f), width = 300f, height = 6f, depth = 6f))
        )
    )
    ...
    override fun build(context: PoseContext): SkeletonPose {
        ...
        // Because of the severe wide angle of the arms, the physical apex of the pull is lower
        val pelvisY = lerp(230f, 360f, lift)
        val pelvisX = lerp(-5f, 5f, lift)

        // Torso remains much more strictly vertical to isolate the upper lats
        val torsoPitch = lerp(0.0f, -0.1f, lift)
        ...
        val headTilt = lerp(0f, -0.2f, lift)
        ...
        // Extra Wide Grip
        val barY = 500f
        val gripWidth = def.shoulderWidth * 2.2f

        val targetHandA = Vector3(pelvisX, barY, -gripWidth)
        val targetHandP = Vector3(pelvisX, barY, gripWidth)

        // Pole vectors flare extremely outwards (+/- 3 on Z)
        val poleArmA = Vector3(0f, -1f, -3f)
        val poleArmP = Vector3(0f, -1f, 3f)
        ...
        // Floating Legs (Knees slightly bent backward)
        val ankleY = pelvisY - 140f
        val ankleX = pelvisX - 30f
        val targetAnkleF = Vector3(ankleX, ankleY, -def.hipWidth * 0.8f)
        val targetAnkleB = Vector3(ankleX, ankleY, def.hipWidth * 0.8f)

        val legFIK = solveIK(hipF!!.worldPosition, targetAnkleF, def.thighLength, def.shinLength, Vector3(1f, -1f, 0f), def.legIKConstraint, legFBuffer)
        val legBIK = solveIK(hipB!!.worldPosition, targetAnkleB, def.thighLength, def.shinLength, Vector3(1f, -1f, 0f), def.legIKConstraint, legBBuffer)
