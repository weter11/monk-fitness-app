...
class UnderhandChinUpPose : PoseBuilder {
    ...
    override fun build(context: PoseContext): SkeletonPose {
        ...
        // Pulls slightly higher than the pull-up to clear the chin over the bar
        val pelvisY = lerp(230f, 395f, lift)
        val pelvisX = lerp(-15f, 25f, lift)

        // Drastic pitch backward to drive the sternum toward the bar
        val torsoPitch = lerp(-0.05f, -0.45f, lift)
        ...
        // Head stays vertically neutral relative to the ground despite the severe torso pitch
        val headDir = rotAround(Vector3(0f, 1f, 0f), Vector3(0f, 0f, 1f), -torsoPitch, Vector3()).normalize()
        ...
        val barY = 500f
        val gripWidth = def.shoulderWidth * 0.9f // Narrow grip

        val targetHandA = Vector3(pelvisX, barY, -gripWidth)
        val targetHandP = Vector3(pelvisX, barY, gripWidth)

        // Underhand chin-up: Pole vectors force elbows strictly FORWARD (+X)
        val poleArmA = Vector3(1f, -1f, 0f)
        val poleArmP = Vector3(1f, -1f, 0f)
        ...
        // Hands wrap under the bar (Palms facing toward body)
        ...
        // 3. Floating Legs (Crossed at Ankles)
        val ankleY = pelvisY - 140f
        val ankleX = pelvisX - 50f

        // Z-coordinates pull to 0f so the ankles cross exactly in the center line
