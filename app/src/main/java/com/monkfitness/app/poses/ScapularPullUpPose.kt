...
class ScapularPullUpPose : PoseBuilder {
    override val metadata = PoseMetadata(
        camera = CameraDefinition(defaultYaw = 1.19f, defaultPitch = 0.22f, defaultZoom = 1.4f),
    ...
    private fun ensureHierarchy(def: SkeletonDefinition) {
        ...
        // Root is Pelvis for perfect hanging mechanics
    ...
    override fun build(context: PoseContext): SkeletonPose {
        ...
        // 1. Core Scapular Lift Positioning
        // Smooth C2 cosine wave mapping progress 0.0 -> 0.5 (scapular pull peak) -> 1.0 (dead hang)
        val u = (1f - cos(context.progress * 2f * PI.toFloat())) * 0.5f

        // Pelvis range 225f to 245f is optimal with zoom 1.4f
        val pelvisY = lerp(225f, 245f, u)
        val pelvisX = 0f
        val torsoPitch = 0.0f
        ...
        // 2. ARM TARGETS (Anchored on the high pull-up bar)
        val barY = 500f
        ...
        // 3. LEG TARGETS (Hanging straight down below the pelvis)
        ...
        // Solve Leg IK
        val legFIK = solveIK(hipF!!.worldPosition, targetAnkleF, def.thighLength, def.shinLength, Vector3(1f, 0f, 0f), def.legIKConstraint, legFBuffer)
        val legBIK = solveIK(hipB!!.worldPosition, targetAnkleB, def.thighLength, def.shinLength, Vector3(1f, 0f, 0f), def.legIKConstraint, legBBuffer)
