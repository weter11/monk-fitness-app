...
class AlternatingForwardLungesPose : PoseBuilder {
    ...
    override fun build(context: PoseContext): SkeletonPose {
        ...
        // Pelvis shifts forward over the center of the widened stance and drops
        val pelvisX = activeDrop * 40f
        val pelvisY = standH - (activeDrop * 65f)
        val leanAngle = activeDrop * 0.15f // Slight torso lean forward to balance
        ...
        // In a forward lunge, the BACK foot's heel lifts.
        val footPitchF = lungeR * 0.8f // When Right is stepping forward, Left is back
        val footPitchB = lungeL * 0.8f // When Left is stepping forward, Right is back
        ...
        // Side A (Left Arm): Swings forward when Right Leg (lungeR) steps forward
        val targetHandA = Vector3(pelvisX + (armSwing * 30f) + 10f, pelvisY + def.torsoLength - 20f + (abs(armSwing) * 10f), -def.shoulderWidth * 1.5f)
        // Side P (Right Arm): Swings backward when Right Leg (lungeR) steps forward
