...
class ProneCobraStretchPose : BasePose() {
    ...
    override fun build(context: PoseContext): SkeletonPose {
        ...
        headDir.set(0.2f, 1f, 0f)
        SkeletonMath.rotAround(headDir, axisZ, headTilt, headDir)
        headDir.normalize()
        ...
        targetAnkleF.set(targetAnkleX, 15f, -def.hipWidth)
        targetAnkleB.set(targetAnkleX, 15f, def.hipWidth)
        ...
        // The arm IK origin is the hierarchy shoulder (chest.worldRotation.angle already equals
        // the legacy manual rotAround), so no mirrored-FK workaround is needed here.
        val reachX = SkeletonMath.lerp(chestW.x, pelvisX - 50f, context.progress)
        val reachY = SkeletonMath.lerp(15f, 40f, context.progress)

        targetHandA.set(reachX, reachY, -def.shoulderWidth * 1.5f)
        targetHandP.set(reachX, reachY, def.shoulderWidth * 1.5f)
