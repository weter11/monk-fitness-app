...
class NeutralGripPullUpPose : PoseBuilder {
    override val metadata = PoseMetadata(
        ...
        environment = EnvironmentDefinition(
            ...
            // Custom Parallel Handles pointing outward along the Z-axis
            props = listOf(
                BoxProp(center = Vector3(0f, 500f, -22f), width = 10f, height = 6f, depth = 60f),
                BoxProp(center = Vector3(0f, 500f, 22f), width = 10f, height = 6f, depth = 60f)
            )
        )
    )
    ...
    override fun build(context: PoseContext): SkeletonPose {
        ...
        val pelvisY = lerp(230f, 395f, lift)
        val pelvisX = lerp(-10f, 20f, lift) // Body arches into the gap between the handles

        // Torso arches aggressively to bring the sternum up
        val torsoPitch = lerp(-0.05f, -0.40f, lift)
        ...
        val headDir = rotAround(Vector3(0f, 1f, 0f), Vector3(0f, 0f, 1f), -torsoPitch - 0.2f, Vector3()).normalize()
        ...
        // Parallel Bar Grip (Shoulder width exactly)
        val barY = 500f
        val gripWidth = def.shoulderWidth

        val targetHandA = Vector3(pelvisX + 15f, barY, -gripWidth)
        val targetHandP = Vector3(pelvisX + 15f, barY, gripWidth)

        // Pole vectors tuck elbows tight to the body and drive them forward (+X)
        val poleArmA = Vector3(1f, -1f, -0.5f)
        val poleArmP = Vector3(1f, -1f, 0.5f)
        ...
        // 3D Wrist Roll: Palms face inward toward each other
        val wrapAxisA = rotAround(Vector3(0f, 0f, 1f), Vector3(1f, 0f, 0f), 1.57f, Vector3())
        val wrapAxisP = rotAround(Vector3(0f, 0f, 1f), Vector3(1f, 0f, 0f), -1.57f, Vector3())
        handA!!.localRotation.set(wrapAxisA, -torsoPitch - 1.57f)
        handP!!.localRotation.set(wrapAxisP, -torsoPitch - 1.57f)
        ...
        // Floating Legs (Knees bend backward, ankles cross for stability)
        val ankleY = pelvisY - 120f
        val ankleX = pelvisX - 45f
        val targetAnkleF = Vector3(ankleX, ankleY, 5f)
        val targetAnkleB = Vector3(ankleX, ankleY, -5f)

        val legFIK = solveIK(hipF!!.worldPosition, targetAnkleF, def.thighLength, def.shinLength, Vector3(1f, -1f, -1f), def.legIKConstraint, legFBuffer)
        val legBIK = solveIK(hipB!!.worldPosition, targetAnkleB, def.thighLength, def.shinLength, Vector3(1f, -1f, 1f), def.legIKConstraint, legBBuffer)
