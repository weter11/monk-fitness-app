...
class AlternatingSideLungesPose : PoseBuilder {
    ...
    override fun build(context: PoseContext): SkeletonPose {
        ...
        // 1. Lateral Branchless Extraction
        val cycle = context.progress * 2f * PI.toFloat()
        val lungeR = max(0f, sin(cycle))  // Right leg (Side B) lunges outward (+Z)
        val lungeL = max(0f, -sin(cycle)) // Left leg (Side F) lunges outward (-Z)
        val activeDrop = lungeR + lungeL

        // 2. 3D Pelvic Hinge Mechanics
        val standH = def.thighLength + def.shinLength + 25f

        // Hips push backward (-X) like a squat while lunging
        val pelvisX = activeDrop * -25f
        val pelvisY = standH - (activeDrop * 50f)
        // Center of mass shifts sideways toward the lunging leg
        val pelvisZ = (lungeR * 50f) - (lungeL * 50f)

        // Heavy torso pitch forward to counterbalance the hips pushing back
        val torsoPitch = activeDrop * 0.45f

        pelvis!!.localPosition = Vector3(pelvisX, pelvisY, pelvisZ)
        pelvis!!.localRotation.set(Vector3(0f, 0f, 1f), -torsoPitch)

        chest!!.localPosition = Vector3(0f, def.torsoLength, 0f)
        // Head tilts up to look straight ahead despite the deep chest lean
        val headDir = rotAround(Vector3(0f, 1f, 0f), Vector3(0f, 0f, 1f), torsoPitch * 0.8f, Vector3()).normalize()
        ...
        // 3. Straight/Bent Leg Kinematics
        val sideStep = 80f
        val targetAnkleB = Vector3(0f, 25f, def.hipWidth + (lungeR * sideStep))
        val targetAnkleF = Vector3(0f, 25f, -def.hipWidth - (lungeL * sideStep))

        // Pole vectors splay slightly outward to accommodate the wide stance
        val legFIK = solveIK(hipF!!.worldPosition, targetAnkleF, def.thighLength, def.shinLength, Vector3(1f, 0f, -1f), def.legIKConstraint, legFBuffer)
        val legBIK = solveIK(hipB!!.worldPosition, targetAnkleB, def.thighLength, def.shinLength, Vector3(1f, 0f, 1f), def.legIKConstraint, legBBuffer)
        ...
        // Both feet remain completely flat on the floor
        ankleF!!.localRotation.set(Vector3(0f, 0f, 1f), torsoPitch); ankleB!!.localRotation.set(Vector3(0f, 0f, 1f), torsoPitch)
        ...
        // 4. Center Clasped Arms
        val chestW = chest!!.worldPosition

        // Hands transition from resting at sides to clasped centrally in front of the chest
        val handTargetX = lerp(pelvisX + 10f, chestW.x + 25f, activeDrop)
        val handTargetY = lerp(pelvisY + 60f, chestW.y - 15f, activeDrop)
        val handTargetZ_A = lerp(pelvisZ - def.shoulderWidth * 1.5f, pelvisZ - 5f, activeDrop)
        val handTargetZ_P = lerp(pelvisZ + def.shoulderWidth * 1.5f, pelvisZ + 5f, activeDrop)

        val targetHandA = Vector3(handTargetX, handTargetY, handTargetZ_A)
        val targetHandP = Vector3(handTargetX, handTargetY, handTargetZ_P)

        val armA = solveIK(shoulderA!!.worldPosition, targetHandA, def.upperArmLength, def.forearmLength, Vector3(0f, -1f, -2f), def.armIKConstraint, armABuffer)
        val armP = solveIK(shoulderP!!.worldPosition, targetHandP, def.upperArmLength, def.forearmLength, Vector3(0f, -1f, 2f), def.armIKConstraint, armPBuffer)
