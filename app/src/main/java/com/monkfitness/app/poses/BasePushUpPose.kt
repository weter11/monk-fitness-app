...
abstract class BasePushUpPose : BasePose() {
    ...
    override fun build(context: PoseContext): SkeletonPose {
        ...
        if (isKneePivot) {
            ...
            // Same as feet-pivot: the body stays rigid, depth comes from elbow flexion.
            // The knee is the floor pivot; the torso must NOT pitch (that caused shoulders
            // to lurch past hands). Instead, the arm IK bends the elbows to lower the chest
            // while the torso stays horizontal.
            val depth = 0.5f - 0.5f * cos(context.progress * 2f * PI.toFloat())
            val armLen = def.upperArmLength + def.forearmLength
            val topH = armLen * 0.92f
            val botH = def.shinLength * 0.55f
            val bodyH = topH + (botH - topH) * depth
            ...
            // 2. Front leg: ankle (root) -> knee (shin, 45° up) -> hip (thigh, rigid).
            ...
            // Rigid leg continuation: shin is at -shinPitch, so femur carries +shinPitch.
            ...
            // B7: single spine curve — rigid plank, no trunk pitch.
            // The old torsoPitch code wrote pelvis + chest independently (B7 violation)
            // and caused shoulders to lurch past hands. Rigid torso keeps shoulders above
            // hands; depth comes from arm IK below.
        ...
        // Head lifts as body descends (looks more forward), drops as body rises (looks
        // more down). sin(progress * PI) peaks at mid-descent, so the Y component
        // increases at the bottom of the rep — the head lifts toward horizontal.
        val headLift = 0.15f * sin(context.progress * PI.toFloat())
        val animatedHeadDir = Vector3(-1f, 0.2f + headLift, 0f).normalize()
        buildGaze(neck!!, head!!, def.neckLength, animatedHeadDir)
