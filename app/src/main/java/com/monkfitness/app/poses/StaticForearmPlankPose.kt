...
class StaticForearmPlankPose : PoseBuilder {
    ...
    override fun build(context: PoseContext): SkeletonPose {
        ...
        val torsoPitch = lerp(-1.57f, -1.5f, lift)
        ...
        val targetAnkleY = lerp(15f, 25f, lift) // Lift heels off floor so weight rests purely on toes

        val targetAnkleF = Vector3(targetAnkleX, targetAnkleY, -def.hipWidth)
        val targetAnkleB = Vector3(targetAnkleX, targetAnkleY, def.hipWidth)
        ...
        val toeFlex = lerp(0f, 0.6f, lift)
        ankleF!!.localRotation.set(Vector3(0f, 0f, 1f), -torsoPitch - toeFlex)
        ankleB!!.localRotation.set(Vector3(0f, 0f, 1f), -torsoPitch - toeFlex)
        ...
        // Hands meet closely in the center (Z ~ 0) to form the stability triangle
        val handTargetX = chestW.x + def.forearmLength * 0.8f
        val handTargetY = 15f

        val targetHandA = Vector3(handTargetX, handTargetY, -5f)
        val targetHandP = Vector3(handTargetX, handTargetY, 5f)
        ...
        val armP = solveIK(shoulderPW, targetHandP, def.upperArmLength, def.forearmLength, Vector3(-1f, -1f, 1f), def.armIKConstraint, armPBuffer)
