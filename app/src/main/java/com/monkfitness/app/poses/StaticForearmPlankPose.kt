...
class StaticForearmPlankPose : BasePose() {
    ...
    private val legPole = Vector3(0f, 1f, 0f)
    private val armAPole = Vector3(-1f, -1f, -1f)
    private val armPPole = Vector3(-1f, -1f, 1f)
    ...
    override fun build(context: PoseContext): SkeletonPose {
        ...
        // 1. Rigid Plank Incline Lift
        val lift = context.progress
        val pelvisX = 0f
        val pelvisY = SkeletonMath.lerp(15f, 35f, lift)

        val torsoPitch = SkeletonMath.lerp(-1.57f, -1.5f, lift)
        ...
        // 2. Straight Legs to Toes
        val targetAnkleX = pelvisX - def.thighLength - def.shinLength + 20f
        val targetAnkleY = SkeletonMath.lerp(15f, 25f, lift) // Lift heels off floor so weight rests purely on toes

        targetAnkleF.set(targetAnkleX, targetAnkleY, -def.hipWidth)
        targetAnkleB.set(targetAnkleX, targetAnkleY, def.hipWidth)

        bakeIkLimb(hipF!!.worldPosition, targetAnkleF, def.thighLength, def.shinLength, legPole, def.legIKConstraint, -torsoPitch, kneeF!!, ankleF!!, legFBuffer)
        bakeIkLimb(hipB!!.worldPosition, targetAnkleB, def.thighLength, def.shinLength, legPole, def.legIKConstraint, -torsoPitch, kneeB!!, ankleB!!, legBBuffer)

        // Plantar flexion forces toes straight into the floor to carry the weight
        val toeFlex = SkeletonMath.lerp(0f, 0.6f, lift)
        ankleF!!.localRotation.set(axisZ, -torsoPitch - toeFlex)
        ankleB!!.localRotation.set(axisZ, -torsoPitch - toeFlex)
        ...
        // 3. Forearm Ground Anchorage
        val chestW = chest!!.worldPosition
        // The arm IK origin (legacy manual rotAround) equals the hierarchy shoulder here, so the
        // engine-owned shoulderA.worldPosition is used directly — no mirrored-FK workaround needed.
        val handTargetX = chestW.x + def.forearmLength * 0.8f
        val handTargetY = 15f

        targetHandA.set(handTargetX, handTargetY, -5f)
        targetHandP.set(handTargetX, handTargetY, 5f)

        // Pole vectors aggressively pull the elbows backward and slightly outward to pin them to the floor
        ...
        // Hands lay perfectly flat on the mat facing forward
