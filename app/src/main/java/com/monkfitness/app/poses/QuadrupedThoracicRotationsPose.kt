...
class QuadrupedThoracicRotationsPose : PoseBuilder {
    ...
    override fun build(context: PoseContext): SkeletonPose {
        ...
        // 1. Tabletop Anchoring
        val basePelvisX = -20f
        val basePelvisY = 127f // Perfect height for horizontal thigh
        val torsoPitch = -PI.toFloat() / 2f // Flattens spine parallel to the floor
        ...
        // 2. 3D Thoracic Twist Implementation
        val twist = lerp(-0.2f, 1.4f, context.progress)
        ...
        val shoulderAW = rotAround(sARoll, Vector3(0f, 0f, 1f), torsoPitch, Vector3()).add(chestW)
        val shoulderPW = rotAround(sPRoll, Vector3(0f, 0f, 1f), torsoPitch, Vector3()).add(chestW)

        // 3. Quadruped Lower Body
        val targetAnkleF = Vector3(basePelvisX - def.shinLength, 15f, -def.hipWidth)
        val targetAnkleB = Vector3(basePelvisX - def.shinLength, 15f, def.hipWidth)
        val legFIK = solveIK(hipF!!.worldPosition, targetAnkleF, def.thighLength, def.shinLength, Vector3(0f, -1f, -0.5f), def.legIKConstraint, legFBuffer)
        val legBIK = solveIK(hipB!!.worldPosition, targetAnkleB, def.thighLength, def.shinLength, Vector3(0f, -1f, 0.5f), def.legIKConstraint, legBBuffer)
        ...
        // 4. Thoracic Arm Mechanics
        // Support Arm (Side P) strictly straight down beneath the natural shoulder line
        val handTargetP = Vector3(basePelvisX + def.torsoLength, 0f, def.shoulderWidth)
        val armPIK = solveIK(shoulderPW, handTargetP, def.upperArmLength, def.forearmLength, Vector3(-1f, 0f, 1f), def.armIKConstraint, armPBuffer)

        // Dynamic Arm (Side A) anchors hand to the back of the neck
        val neckW = neck!!.worldPosition
        val handTargetA = Vector3(neckW.x, neckW.y + 10f, 0f) // Pulled behind the neck

        // Dynamically rotate the IK pole vector so the elbow physically sweeps open as the chest twists
        val basePoleA = Vector3(0f, -1f, -1f)
        val sweptPoleA = rotAround(basePoleA, Vector3(1f, 0f, 0f), twist, Vector3())
        val armAIK = solveIK(shoulderAW, handTargetA, def.upperArmLength, def.forearmLength, sweptPoleA, def.armIKConstraint, armABuffer)

        shoulderA!!.localPosition.set(sARoll.x, sARoll.y, sARoll.z)
        shoulderP!!.localPosition.set(sPRoll.x, sPRoll.y, sPRoll.z)

        rotAround(Vector3(armAIK.joint.x - shoulderAW.x, armAIK.joint.y - shoulderAW.y, armAIK.joint.z - shoulderAW.z), Vector3(0f, 0f, 1f), -torsoPitch, elbowA!!.localPosition)
        rotAround(Vector3(armAIK.end.x - armAIK.joint.x, armAIK.end.y - armAIK.joint.y, armAIK.end.z - armAIK.joint.z), Vector3(0f, 0f, 1f), -torsoPitch, handA!!.localPosition)
        rotAround(Vector3(armPIK.joint.x - shoulderPW.x, armPIK.joint.y - shoulderPW.y, armPIK.joint.z - shoulderPW.z), Vector3(0f, 0f, 1f), -torsoPitch, elbowP!!.localPosition)
        rotAround(Vector3(armPIK.end.x - armPIK.joint.x, armPIK.end.y - armPIK.joint.y, armPIK.end.z - armPIK.joint.z), Vector3(0f, 0f, 1f), -torsoPitch, handP!!.localPosition)
