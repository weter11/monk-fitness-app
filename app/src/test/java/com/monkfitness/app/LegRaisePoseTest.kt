...
class LegRaisePoseTest {

    private val def = SkeletonDefinition.DEFAULT_ADULT
    private val legRaisePose = LegRaisePose()
    private val finalizer = SkeletonPoseFinalizer(def)
    private val validator = ExerciseValidator()
    private val camera = Camera(legRaisePose.metadata.camera)

    @Test
    fun testLegRaisePoseMeetsAllBiomechanicalRequirements() {
        val totalFrames = 90
        val dt = 0.033f
        ...
        for (i in 0..totalFrames) {
            ...
            val rawPose = legRaisePose.build(context)
            ...
            val report = validator.validate(
                pose = pose,
                definition = def,
                environment = legRaisePose.metadata.environment,
                camera = camera,
                width = 1000f,
                height = 1000f,
        ...
        println("Validated Leg Raise sequence successfully! Exercise score: ${reviewReport.score}")
