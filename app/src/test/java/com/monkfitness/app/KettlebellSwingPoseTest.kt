...
class KettlebellSwingPoseTest {

    private val def = SkeletonDefinition.DEFAULT_ADULT
    private val swingPose = KettlebellSwingPose()
    private val finalizer = SkeletonPoseFinalizer(def)
    private val validator = ExerciseValidator()
    private val camera = Camera(swingPose.metadata.camera)

    @Test
    fun testKettlebellSwingPoseMeetsAllBiomechanicalRequirements() {
        val totalFrames = 90
        val dt = 0.033f
        ...
        for (i in 0..totalFrames) {
            ...
            val rawPose = swingPose.build(context)
            ...
            val report = validator.validate(
                pose = pose,
                definition = def,
                environment = swingPose.metadata.environment,
                camera = camera,
                width = 1000f,
                height = 1000f,
        ...
        println("Validated Kettlebell Swing sequence successfully! Exercise score: ${reviewReport.score}")
