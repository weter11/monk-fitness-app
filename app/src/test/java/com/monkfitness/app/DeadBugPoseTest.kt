...
class DeadBugPoseTest {

    private val def = SkeletonDefinition.DEFAULT_ADULT
    private val deadBugPose = DeadBugPose()
    private val pipeline = SkeletonPipeline(def)
    private val validator = ExerciseValidator()
    private val camera = Camera(deadBugPose.metadata.camera)

    @Test
    fun testDeadBugPoseMeetsAllBiomechanicalRequirements() {
        val totalFrames = 90
        val dt = 0.033f
        ...
        for (i in 0..totalFrames) {
            ...
            val rawPose = deadBugPose.build(context)
            ...
            val report = validator.validate(
                pose = pose,
                definition = def,
                environment = deadBugPose.metadata.environment,
                camera = camera,
                width = 1000f,
                height = 1000f,
