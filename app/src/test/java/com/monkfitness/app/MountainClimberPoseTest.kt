...
class MountainClimberPoseTest {

    private val def = SkeletonDefinition.DEFAULT_ADULT
    private val climberPose = MountainClimberPose()
    private val pipeline = SkeletonPipeline(def)
    private val validator = ExerciseValidator()
    private val camera = Camera(climberPose.metadata.camera)

    @Test
    fun testMountainClimberPoseMeetsAllBiomechanicalRequirements() {
        val totalFrames = 90
        val dt = 0.033f
        ...
        for (i in 0..totalFrames) {
            ...
            val rawPose = climberPose.build(context)
            ...
            val report = validator.validate(
                pose = pose,
                definition = def,
                environment = climberPose.metadata.environment,
                camera = camera,
                width = 1000f,
                height = 1000f,
        ...
        println("Validated Mountain Climber sequence successfully! Exercise score: ${reviewReport.score}")
