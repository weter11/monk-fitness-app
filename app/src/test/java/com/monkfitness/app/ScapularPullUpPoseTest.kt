...
import com.monkfitness.app.poses.ScapularPullUpPose
import org.junit.Assert.*
import org.junit.Test
import kotlin.math.*

class ScapularPullUpPoseTest {

    private val def = SkeletonDefinition.DEFAULT_ADULT
    private val pullUpPose = ScapularPullUpPose()
    private val pipeline = SkeletonPipeline(def)
    private val validator = ExerciseValidator()
    private val camera = Camera(pullUpPose.metadata.camera)

    @Test
    fun testScapularPullUpPoseMeetsAllBiomechanicalRequirements() {
        val totalFrames = 90
        val dt = 0.033f
        ...
        for (i in 0..totalFrames) {
            ...
            val rawPose = pullUpPose.build(context)
            ...
            val report = validator.validate(
                pose = pose,
                definition = def,
                environment = pullUpPose.metadata.environment,
                camera = camera,
                width = 1000f,
                height = 1000f,
        ...
        println("Validated Scapular Pull-up sequence successfully! Exercise score: ${reviewReport.score}")
