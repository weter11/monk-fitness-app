...
- specify workflow steps (→ `RFC_MONKENGINE_ENGINEERING_PLAYBOOK`, i.e. `RFC_MONKENGINE_POSE_DEVELOPMENT_PROTOCOL.md`),
...
a unit of work** — it is a raw request awaiting intake. Intake is performed exclusively by the
Development Orchestrator (`RFC_MONKENGINE_DEVELOPMENT_ORCHESTRATOR.md` §1).
...
1. **Intake by the Orchestrator** (§2) — the task is received and owned.
2. **Classification** of the task — category + capability level
...
3. **Specification of required inputs** — which ACTIVE documents, specs, experts, and validation the
   level demands (`RFC_MONKENGINE_DEVELOPMENT_ORCHESTRATOR.md` §2.3–§2.4;
...
5. **Approval of the Execution Plan** (§8) — the plan is complete and approved.
...
- the **Required Workflow** runs as the work sequence
  (`RFC_MONKENGINE_ENGINEERING_PLAYBOOK` / `RFC_MONKENGINE_POSE_DEVELOPMENT_PROTOCOL.md`, selected by
  level),
...
Allowing implementation before plan approval is a violation of this contract, not a shortcut.
...
- **Required Workflow** — the `RFC_MONKENGINE_ENGINEERING_PLAYBOOK`
  (`RFC_MONKENGINE_POSE_DEVELOPMENT_PROTOCOL.md`) sequence the level selects.
...
Approval is the Orchestrator's decision that the plan is sufficient to bound the work. It is the
single release point for implementation.

---

## 8. Implementation

Implementation executes the approved plan (§4, §6–§7):
...
After implementation, the **Required Verification** (§6.1) is executed: the mandatory final
verification that proves the work meets its acceptance bar.
...
Acceptance is the gate owned by `RFC_MONKENGINE_DEFINITION_OF_DONE.md`. The task is accepted only
when:
...
After acceptance, the **Required knowledge capture** is performed: the durable record the task must
leave is promoted through governance, and all temporary expert artifacts are destroyed
...
RFC_MONKENGINE_DEVELOPMENT_SYSTEM.md          (the system map; this contract is a node under it)
        │
RFC_MONKENGINE_DEVELOPMENT_ORCHESTRATOR.md    (receives the task; produces the plan — §2, §3, §6)
        │
RFC_MONKENGINE_CAPABILITY_LEVELS.md           (classifies the task; bounds the plan — §3, §6)
        │
RFC_MONKENGINE_ENGINEERING_PLAYBOOK
  (= RFC_MONKENGINE_POSE_DEVELOPMENT_PROTOCOL.md)  (the Required Workflow the plan selects — §6, §8)
        │
...
- **DEVELOPMENT_ORCHESTRATOR** — the sole receiver (§2); the producer of the Execution Plan (§6) and
  the approver/releaser (§7). This contract adds no decision content; it makes the Orchestrator's
  plan *mandatory before implementation*.
- **CAPABILITY_LEVELS** — supplies the classification and capability the plan must contain (§6.1).
- **ENGINEERING_PLAYBOOK** (PDP) — supplies the Required Workflow the approved plan executes (§8).
