...
- After any pose change, confirm `./gradlew :app:testDebugUnitTest` stays at 0 failures against the
  ...
  measured fresh in this pass's own base worktree; `123 / 599` with the B1 diamond elbow-plane correction.
  Earlier standing points: the T2 instrument branch `122 / 592`, the M11/M12 limb-realization migration
  `120 / 577`, the M13 hamstring-reach correction `119 / 572`, the M6/M7 landing plus the M8/M9/M10
  declaration pass `118 / 566`; `--rerun-tasks` with the results directory purged, XML-stamped fresh; the
