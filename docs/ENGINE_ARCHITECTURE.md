...
| **ConstraintSolver** | Runs after IK, before FK. Reconciles the authored pose with its declared support contacts via damped CCD, regularized toward the authored shape; applies only lateral pelvis roll for balance. | `docs/ENGINE_ARCHITECTURE.md` §R4; `docs/ENGINE.md` §5 (local vs world); `docs/HISTORICAL/ENGINE_HISTORY.md` (UNI-1/UNI-4) |
...
| **Pelvic / Hip Complex** | Focused architectural investigation of the pelvis and hip joints, ROM limits, and the ConstraintSolver tilt axis. The historical Q&A and issue list live in the archive; the consolidated status is in the history index. (Note: there is no separate `HIP_COMPLEX.md`; hip material is `docs/HISTORICAL/PELVIC_HIP_COMPLEX_INVESTIGATION.md`, reached via `docs/HISTORICAL/ENGINE_HISTORY.md`.) | `docs/HISTORICAL/ENGINE_HISTORY.md` §4; `docs/HISTORICAL/ENGINE_ROADMAP.md` |
| **Engineering Validation** | The hidden developer category of frozen reference poses used to verify the MonkEngine runtime. Includes the four-pose defect audit and camera framing pass. | `docs/VALIDATION.md` §4–5, §11; `docs/HISTORICAL/ENGINE_HISTORY.md` §3, §5 |
| **History / Archive** | The full historical investigation, audit, and fix-prompt records (UNI-1…12, per-pose audits, pelvic/hip Q&A, PR prompts). Retained as evidence, not kept in sync with code. | `docs/HISTORICAL/ENGINE_HISTORY.md` |
| **Roadmap / Future work** | The live list of remaining engine work (open UNI items, trunk DOFs, generalized contacts). | `docs/HISTORICAL/ENGINE_ROADMAP.md` |
...
```
  ...
  (how it works)  (movement rules) (standing rules) (verify + poses)  (known test state)
        ...
        └──────┬───────┴──────────────┬───────────────┘
               ▼                      ▼
     docs/HISTORICAL/ENGINE_HISTORY.md         docs/HISTORICAL/ENGINE_ROADMAP.md
...
- **Engine/core change:** `ENGINE.md` §11–12 (what belongs inside) → relevant
  ...
  regressions.
- **History / past decisions:** `docs/HISTORICAL/ENGINE_HISTORY.md` (index) → the four detailed
  HISTORICAL records it links to.
- **What's left to do:** `docs/HISTORICAL/ENGINE_ROADMAP.md` (live open items + resolved list).
- **Hip/pelvis work:** `docs/HISTORICAL/ENGINE_HISTORY.md` §4 (Q&A verdicts) and §2 (UNI status);
  for live hip work see `docs/HISTORICAL/ENGINE_ROADMAP.md`.
- **Validation/audit:** `VALIDATION.md` → `docs/HISTORICAL/ENGINE_HISTORY.md` §3, §5 (audit index).
